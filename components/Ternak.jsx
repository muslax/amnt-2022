...
export default function Ternak ({ idr, editable }) {
    const { data, error } = useSWR(`/api/get?q=ternak&idr=${idr}`, fetchJson)
    ...
    async function saveItem() {
        setSubmitting(true)
        try {
            await fetchJson(`/api/post?q=save-ternak`, generatePOSTData(model))
            mutate(`/api/get?q=ternak&idr=${idr}`)
        } catch (error) {
            alert("ERROR")
        }
        setAdd(false)
    ...
    async function deleteItem(id) {
        try {
            await fetchJson(`/api/post?q=delete-ternak&id=${id}`, generatePOSTData(model))
            mutate(`/api/get?q=ternak&idr=${idr}`)
        } catch (error) {
            alert("ERROR")
        }
    ...
    return (
        <div className="my-10">
            <h3 className="text-blue-500 font-bold mb-3">Hewan Ternak</h3>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-gray-400 text-white whitespace-nowrap">
                            ...
                            <td className="p-2 border-l">Jumlah<br/>(ekor)</td>
                            <td className="p-2 border-l">Dikonsumsi<br/>ekor/tahun</td>
                            <td className="p-2 border-l">Dijual<br/>ekor/tahun</td>
                    ...
                    {daftar.map((item, index) => (
                        ...
                        >
                            ...
                            <td className="p-2 border-l border-gray-300">{item.jumlah}</td>
            ...
            {! add && editable && (
                <div className="my-4">
                    <button 
                    onClick={e => {
                        setModel(NewTernak(idr))
                        setProxy(NewTernak(idr))
                        setAdd(true)
                    }} 
